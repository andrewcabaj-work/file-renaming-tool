...
def handle_mid_start_operation(string, n):
    directory_path = folder_path_entry.get()
    start_renaming_thread('file-rn-mid-start v3.py', directory_path, f"{string},{n}")

def mid_start_operation():
    string = mid_start_string_entry.get()
    n = mid_start_n_entry.get()
    if n.isdigit():
        handle_mid_start_operation(string, int(n))
    else:
...
# Add UI elements for mid-start operation
mid_start_label = tk.Label(root, text="Insert String at Position", font=('Helvetica', 8, 'bold'))
mid_start_label.grid(row=4, column=0, sticky='W', padx=5, pady=(5,0))

mid_start_string_entry = tk.Entry(root, width=20)
mid_start_string_entry.grid(row=4, column=1, padx=5, pady=(5,0), sticky='ew')

mid_start_n_entry = tk.Entry(root, width=5)
mid_start_n_entry.grid(row=4, column=2, padx=5, pady=(5,0), sticky='ew')

mid_start_button = tk.Button(root, text="INSERT", command=mid_start_operation, bg='#A9A9A9', font=('Helvetica', 7, 'bold'))
mid_start_button.grid(row=4, column=3, padx=5, pady=(5,0), sticky='ew')
